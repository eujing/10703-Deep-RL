...
Original file is located at
    https://colab.research.google.com/drive/1TMd_9G4LLvagLzLbHGivkey7gf954zHw
...
To implement behavior cloning and DAGGER, fill in the missing blocks of code below. The provided code loads an expert model upon creation of the `Imitation` class. The function `generate_behavior_cloning_data()` fills in `self._train_states` and `self._train_actions` with states and actions from a single episode. Later, when implementing DAGGER, you will finish implementing the `generate_dagger_data()` which should overwrite `self._train_actions`.
"""
...
class Imitation():
    ...
    def generate_dagger_data(self):
        ...
        for _ in range(self.num_episodes):
            ...
            correct_actions = np.argmax(self.expert.predict(states), axis=1)
            
            # OHE the actions
            actions_ohe = np.zeros_like(actions)
            actions_ohe[np.arange(actions.shape[0]), correct_actions] = 1
            
            self._train_states.extend(states)
            self._train_actions.extend(actions_ohe)
        self._train_states = np.array(self._train_states)
        self._train_actions = np.array(self._train_actions)
    ...
    def train(self, num_epochs=200, render=False):
        ...
        """
        history = self.model.fit(self._train_states, self._train_actions,
...
for num_episodes in [1, 10, 50, 100]:
  for t in range(random_seeds):
    ...
    for i in range(num_iterations):
        ...
        reward_data[num_episodes].append(reward)
        if i % 10 == 0:
          print('\t(%d) loss = %.3f; accuracy = %.2f; reward = %.1f' % (i, loss, acc, reward))

"""Plot the reward, loss, and accuracy for each, remembering to label each line."""

keys = sorted([1, 10, 50, 100])
# plt.figure(figsize=(12, 4))
# for (index, (data, name)) in enumerate(zip([reward_data, accuracy_data, loss_data],
#                                            ['reward', 'accuracy', 'loss'])):
#   plt.subplot(1, 3, index + 1)
#   # plt.plot([float(i) for i in data.keys()], data.values(), '-o')
#   # plt.xlabel('number of expert trajectories', fontsize=16)
#   # plt.ylabel('name', fontsize=16)
# plt.savefig('expert_data_%s.png' % mode, dpi=300)
# plt.show()

def plot_many(data, color):
  for traj in data:
    plt.plot(traj, alpha=0.3, c=color)
  plt.plot(data.mean(axis=0), color)

plt.figure(figsize=(12, 10))
for (index, num_eps) in enumerate(keys):
  plt.subplot(4, 3, 3 * index + 1)
  rewards = np.array(reward_data[num_eps]).reshape(5, 100)
  if index == 0:
    plt.title("Reward")
  plt.ylabel("%d Episodes" % num_eps)
  if index == 3:
    plt.xlabel("Iterations")
  plot_many(rewards, "C0")

  plt.subplot(4, 3, 3 * index + 2)
  accuracies = np.array(accuracy_data[num_eps]).reshape(5, 100)
  if index == 0:
    plt.title("Accuracy")
  if index == 3:
    plt.xlabel("Iterations")
  plot_many(accuracies, "C1")

  plt.subplot(4, 3, 3 * index + 3)
  losses = np.array(loss_data[num_eps]).reshape(5, 100)
  if index == 0:
    plt.title("Loss")
  if index == 3:
    plt.xlabel("Iterations")
...
class CMAES:
    ...
    def populate(self):
        ...
        self.population = []
        # WRITE CODE HERE
        self.population = multivariate_normal(self.mu, self.S).rvs(self.n)
    ...
    def evaluate_policy(self, policy, num_episodes):
        ...
        for episode in range(num_episodes):
          # WRITE CODE HERE
          states, actions, r = generate_episode(self.env, policy)
          if self.reward_fn is None:
            reward = sum(r)
          else:
            rewards = np.array([self.reward_fn(s, a) for s, a in zip(states, actions)])
    ...
    def train(self):
        ...
        member_scores = [self.evaluate(m, self.L) for m in self.population]
        top_scores_idx = np.flip(np.argsort(member_scores))[:int(self.n * self.p)]
        top_members = self.population[top_scores_idx]
        best_member = top_members[0]

        # Update mu
        self.mu = np.mean(top_members, axis=0)

        # Update S
        self.S = np.cov(top_members, rowvar=False) + np.eye(self.d) * self.noise
...
avg_r = np.array(values)[:, 0]
x = np.arange(len(best_r)) + 1

plt.plot(x, best_r, label="Best")
plt.plot(x, avg_r, label="Avg.")
...
plt.legend()
plt.tight_layout()
plt.savefig('cmaes_avg_best.png')
plt.show()
...
class GAIL(object):
  ...
  def collect_data(self, num_episodes):
    ...
    for _ in range(num_episodes):
      ...
      if collect_expert:
        states, _, _ = generate_episode(self.env, self.expert)
      ...
      states, _, _ = generate_episode(self.env, self.cmaes.model)
      self.student_states.extend(states)

    # Balance out training data
    # while len(self.student_states) < len(self.expert_states):
    #   states, _, _ = generate_episode(self.env, self.cmaes.model)
    #   self.student_states.extend(states)
  ...
  def train_discriminator(self):
    ...
    Y[len(self.expert_states):, 1] = 1
    
    assert Y.shape[1] == 2  # Use a 1-hot encoding for the labels
    assert np.all(np.sum(Y, axis=1) == 1)
    history = self.discriminator.fit(X, Y, epochs=20, batch_size=256, verbose=0)
    loss = history.history['loss'][-1]
    acc = history.history['acc'][-1]
    return loss, acc
...
if gail_mode == "disc_only" or gail_mode == "gen_only":
  for t in range(10):
    gail.collect_data(50)
    loss, acc = gail.train_discriminator()
    disc_loss.append(loss)
    disc_acc.append(acc)
    print(f"{t} Loss = {loss:.5f}, Accuracy = {acc:.5f}")

if gail_mode != "disc_only":
  for t in range(100):
    # WRITE CODE HERE

    gail.collect_data(50)
    
    if gail_mode == "full" and t % discriminator_update_period == 0:
      loss, acc = gail.train_discriminator()
      disc_loss.append(loss)
      disc_acc.append(acc)
      print(f"({t}) Loss = {loss:.5f}, Accuracy = {acc:.5f}")
    ...
    # print('(%d) avg member rew = %.5f; best member rew = %.5f' % (t, mu_r, best_r))

    p_expert, avg_r, tv_dist = evaluate(gail)
    gen_p_expert.append(p_expert)
    gen_avg_r.append(avg_r)
    gen_tv_dist.append(tv_dist)

    print('(%d) Policy: p(expert) = %.2f%% ; reward = %.1f' % (t, 100.0 * p_expert, avg_r))
    print('(%d) Total variation distance = %.2f' % (t, tv_dist))

x = np.arange(len(disc_loss)) + 1
plt.plot(x, disc_acc)
plt.xlabel("Num. Iteration", fontsize=16)
plt.ylabel("Accuracy", fontsize=16)
plt.tight_layout()
plt.savefig('gail_disc_acc.png')
plt.show()

x = np.arange(len(gen_p_expert)) + 1

plt.figure(figsize=(12, 4))
plt.subplot(131)

plt.plot(x, gen_p_expert)
plt.xlabel("Num. Iteration", fontsize=16)
plt.ylabel("P(Expert)", fontsize=16)

plt.subplot(132)

plt.plot(x, gen_avg_r)
plt.xlabel("Num. Iteration", fontsize=16)
plt.ylabel("Avg. Reward", fontsize=16)

plt.subplot(133)

plt.plot(x, gen_tv_dist)
plt.xlabel("Num. Iteration", fontsize=16)
plt.ylabel("TV Distance", fontsize=16)

plt.tight_layout()
plt.savefig(f'gail_{gail_mode}.png')
plt.show()
