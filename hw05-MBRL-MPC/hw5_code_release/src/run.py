...
class ExperimentModelDynamics:
    ...
    def train(self, num_train_epochs, num_episodes_per_epoch, evaluation_interval):
        ...
        cem_succ_rates = []
        rand_succ_rates = []
        samples = collections.deque(maxlen=5000)

        # Fill replay memory with 100 episodes again
        for i in range(100):
            samples.append(self.agent.sample(self.task_horizon, self.random_policy_no_mpc))
        ...
        for i in range(num_train_epochs):
            ...
            print("Rewards obtained:", samples[-1]["reward_sum"])

            losses, rmses = self.cem_policy.train(
                [sample["obs"] for sample in samples],
                [sample["ac"] for sample in samples],
                [sample["rewards"] for sample in samples],
                epochs=5
...
def test_cem_gt_dynamics(num_episode=10):
    ...
    print('CEM Only PushingEnv: avg_reward: {}, avg_success: {}'.format(avg_reward, avg_success))

    mpc_params = {'use_mpc': False, 'num_particles': 1}
    exp = ExperimentGTDynamics(env_name='Pushing2D-v1', mpc_params=mpc_params)
    avg_reward, avg_success = exp.test(num_episode, optimizer="random")
    print('Random Only PushingEnv: avg_reward: {}, avg_success: {}'.format(avg_reward, avg_success))
    ...
    avg_reward, avg_success = exp.test(num_episode)
    print('CEM Only PushingEnv Noisy: avg_reward: {}, avg_success: {}'.format(avg_reward, avg_success))
...
def train_single_dynamics(num_test_episode=50):
    ...
    losses, rmses = exp.model_warmup(num_episodes=num_episodes, num_epochs=num_epochs)

    with open("single_dynamics_warmup.json", "w") as f:
        json.dump({"losses": losses, "rmses": rmses}, f)
...
def train_pets():
    ...
    mpc_params = {'use_mpc': True, 'num_particles': 6}
    exp = ExperimentModelDynamics(env_name='Pushing2D-v1', num_nets=num_nets, mpc_params=mpc_params)
    warmup_losses, warmup_rmses = exp.model_warmup(num_episodes=100, num_epochs=10)
    losses, rmses, cem_rates, rand_rates = exp.train(num_train_epochs=num_epochs,
              num_episodes_per_epoch=num_episodes_per_epoch,
              evaluation_interval=evaluation_interval)

    with open("mbrl_pets.json", "w") as f:
        json.dump({
            "warmup_losses": warmup_losses,
            "warmup_rmses": warmup_rmses,
            "losses": losses,
            "rmses": rmses,
            "cem_succ_rates": cem_rates,
            "rand_succ_rates": rand_rates
        }, f)
